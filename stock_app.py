...
# 3. AI 분석 함수 (Gemini)
def ask_ai_analysis(content):
    """AI에게 공시 내용 분석을 요청합니다."""
    try:
        genai.configure(api_key=st.secrets["GEMINI_API_KEY"])
        model = genai.GenerativeModel('gemini-1.5-flash')
        prompt = f"너는 주식 전문가야. 다음 공시 제목을 읽고 투자자에게 호재인지 악재인지 판단해서 1문장으로 핵심만 요약해줘: {content}"
        ...
        if df is not None and not df.empty:
            for _, row in df.iterrows():
                ...
                for key in positive_keywords:
                    if key in title:
                        # 🤖 AI 의견 듣기
                        ai_opinion = ask_ai_analysis(title)
        ...
        for ticker in df_top.index:
            try:
                ...
                if df is not None and len(df) > 20:
                    price_data = df.iloc[:, 3] # 종가
                    vol_data = df.iloc[:, 4]   # 거래량
